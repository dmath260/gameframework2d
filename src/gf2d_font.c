...
GFC_FontTypes gf2d_font_type_from_text(const char *buf)
{
...
void gf2d_fonts_load(const char *filename)
{
    ...
    memset(font_manager.font_list,0,sizeof(GFC_Font)*count);
    for (i = 0 ; i < FT_MAX; i++)
    {
...
GFC_Font *gf2d_font_get_by_filename(char *filename)
{
    int i;
...
GFC_Font *gf2d_font_get_by_tag(GFC_FontTypes tag)
{
    if ((tag < 0) ||(tag >= FT_MAX))
    {
...
void gf2d_font_draw_line_tag(char *text, GFC_FontTypes tag, GFC_Color color, GFC_Vector2D position)
{
    gf2d_font_draw_line(text,gf2d_font_get_by_tag(tag),color, position);
}

void gf2d_font_draw_line(char *text, GFC_Font *font, GFC_Color color, GFC_Vector2D position)
{
    SDL_Surface *surface;
    SDL_Texture *texture;
    SDL_Rect dst = {0};
    ...
    SDL_UpdateTexture(texture,
                    ...
                    surface->pixels,
                    surface->pitch);
    gfc_vector2d_copy(dst,position);
    dst.w = surface->w;
    dst.h = surface->h;
    SDL_RenderCopy(
...
GFC_Vector2D gf2d_font_get_bounds_tag(char *text, GFC_FontTypes tag)
{
    return gf2d_font_get_bounds(text,gf2d_font_get_by_tag(tag));
}

GFC_Vector2D gf2d_font_get_bounds(char *text, GFC_Font *font)
{
    int x = -1,y = -1;
    if (!text)
    {
...
void gf2d_font_draw_text_wrap_tag(char *text, GFC_FontTypes tag, GFC_Color color, GFC_Rect block)
{
