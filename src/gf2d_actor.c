...
void gf2d_actor_delete(GFC_Actor *actor)
{
    if (!actor)return;
    gf2d_action_list_delete(actor->al);
    if (actor->sprite)gf2d_sprite_free(actor->sprite);
    memset(actor,0,sizeof(GFC_Actor));
}

void gf2d_actor_free(GFC_Actor *actor)
{
    if (!actor) return;
    actor->_refCount--;
}
...
GFC_Actor *gf2d_actor_get_by_filename(const char * filename)
{
    int i;
    if (!filename)
    {
...
GFC_Action *gf2d_action_new()
{
    return gfc_allocate_array(sizeof(GFC_Action),1);
}

GFC_Action *gf2d_action_json_parse(
    SJson *actionSJ
)
{
    GFC_Action *action;
    ...
    action = gfc_allocate_array(sizeof(GFC_Action),1);
...
SJson *gf2d_action_to_json(GFC_Action *action)
{
    SJson *save;
    if (!action)return NULL;
    save = sj_object_new();
...
SJson *gf2d_actor_to_json(GFC_Actor *actor)
{
    SJson *save;
...
void gf2d_actor_save(GFC_Actor *actor,const char *filename)
{
    SJson *save,*actorjs;
...
GFC_List *gf2d_action_list_parse(GFC_List *al,SJson *actionList)
{
    SJson *item;
    int i,c;
...
GFC_Actor *gf2d_actor_load_json(
    SJson *json,
    const char *filename
)
{
...
GFC_Action *gf2d_action_list_get_action(GFC_List *al, const char *name)
{
    GFC_Action *action;
    int i,c;
    if (!al)
...
Uint32 gf2d_action_next_frame_after(GFC_Action *action,float frame)
{
    Uint32 ret = 0;
    if (!action)return 0;
...
ActionReturnType gf2d_action_next_frame(GFC_Action *action,float *frame)
{
    if ((!action)||(!frame))
    {
        return ART_ERROR;
...
GFC_Actor *gf2d_actor_load_image(const char *file)
{
    GFC_Actor *actor;
    GFC_Action *action;
    Sprite *sprite;
    ...
    actor->size = gfc_vector2d(sprite->frame_w,sprite->frame_h);
    actor->scale = gfc_vector2d(1,1);
    actor->center = gfc_vector2d(sprite->frame_w*0.5,sprite->frame_h *0.5);
...
//    actor->drawOffset;  left zero
    ...
    gfc_list_append(actor->al,action);
    return actor;
}

GFC_Actor *gf2d_actor_load(const char *file)
{
    SJson *json;
    GFC_Actor *actor;
...
GFC_Action *gf2d_actor_set_action(GFC_Actor *actor, const char *name,float *frame)
{
    GFC_Action *action;
    if (!actor)return NULL;
...
GFC_Action *gf2d_action_list_get_action_by_frame(GFC_List *list,Uint32 frame)
{
    GFC_Action *action;
    int i,c;
    if (!list)return NULL;
...
GFC_Action *gf2d_action_list_get_action_by_name(GFC_List *list,const char *name)
{
    GFC_Action *action;
    int i,c;
...
GFC_Action *gf2d_actor_get_action_by_name(GFC_Actor *actor,const char *name)
{
    if (!actor)return NULL;
    return gf2d_action_list_get_action_by_name(actor->al,name);
}

Uint32 gf2d_actor_get_action_count(GFC_Actor *actor)
{
    if (!actor)return 0;
    return gfc_list_get_count(actor->al);
}

GFC_Action *gf2d_actor_get_action_by_index(GFC_Actor *actor,Uint32 index)
{
    if (!actor)return NULL;
    return gfc_list_get_nth(actor->al,index);
}

GFC_Action *gf2d_actor_get_next_action(GFC_Actor *actor, GFC_Action *action)
{
    int i;
    if (!actor)return NULL;
...
Uint32 gf2d_action_get_framecount(GFC_Action *action)
{
    if (!action)return 0;
    return action->endFrame - action->startFrame;
}

Uint32 gf2d_actor_get_framecount(GFC_Actor *actor)
{
    GFC_Action *action;
    int i,c;
    Uint32 count = 0;
...
{
    GFC_Color drawColor;
    GFC_Vector2D drawCenter = {0};
    GFC_Vector2D drawScale = {0};
...
int gf2d_action_get_animation_frames(GFC_Action *action)
{
    if (!action)
    {
        return -1;
...
int gf2d_action_get_percent_complete(GFC_Action *action,float frame)
{
    float total,passed;
    if (!action)
    {
...
int gf2d_action_get_frames_remaining(GFC_Action *action,float frame)
{
    float total,passed;
    if (!action)
    {
...
int gf2d_action_get_action_frame(GFC_Action *action,float frame)
{
    if (!action)
    {
        return -1;
